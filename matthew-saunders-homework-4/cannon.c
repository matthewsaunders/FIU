...
int main(int argc, char* argv[])
{
  ...
  MPI_Cart_coords(comm, id, 2, coord);
	...
	/* Test matrix input files for access */
	for(i=1; i<=2; i++){
		if(!(f = fopen(argv[i], "r")))
			my_abort("Error: Unable to open input file \'%s\'\n", argv[i]);
		fclose(f);
	}
  ...
  printf("id=%d, coord[%d,%d]: read submatrix of A of dims %dx%d\n", id, coord[0], coord[1], ma, na);
	...
	/* Check matrix A for square dimensions */
	a_sqrt = sqrt(ma);
	if(a_sqrt*a_sqrt != ma)
		my_abort("Error: number of rows/columns (m=n=%d) of matrix A must be a square number\n", ma);
  ...
  printf("id=%d, coord[%d,%d]: read submatrix of B of dims %dx%d\n", id, coord[0], coord[1], mb, nb);
	...
	/* Check matrix B for square dimensions */
	b_sqrt = sqrt(mb);
	if(b_sqrt*b_sqrt != mb)
		my_abort("Error: number of rows/columns (m=n=%d) of matrix B must be a square number\n", mb);
	
	/* Check matrix A and B for equal dimensions */
	if(ma != mb)
		my_abort("Error: dimensions [m x n] of matrix A [%dx%d] and B [%dx%d] must be the same\n", ma, na, mb, nb);
	/* Check that sqrt(p) can evenly divide number of rows/columns evenly */
