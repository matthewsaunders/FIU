		...
		try {
			...
			if (result > total) {
				...
				st = (PreparedStatement) connection
						.prepareStatement("SELECT comment_id,comment_family,content,fname,lname FROM Comments,Users where created_by = username Order by comment_id Desc Limit ?");
				st.setInt(1, result);
				rs = st.executeQuery();
				if (rs != null) {
					json = "[";
