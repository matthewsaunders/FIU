...
unsigned char dns_server_ip[100];
...
int main (int argc, char* argv []){
  ...
  strcpy((char*)dns_server_ip, argv[1]);
  strcpy((char*)domain_name, argv[2]);

  //gethostbyname(domain_name, dns_server_ip);
  ngethostbyname(domain_name, Type_A);
...
void ngethostbyname(unsigned char *host , int query_type)
{
    ...
    qinfo =(struct DNS_QUESTION*)&buf[sizeof(struct DNS_HEADER) + (strlen((const char*)qname) + 1)]; //fill it
